...
public abstract class Utility {
	 ...
	 * Read by methodSignatureToString(). Set by side effect,but only
	 * internally.
	 */
	private static boolean wide = false; /*
										 * The `WIDE' instruction is used in the
										 * byte code to allow 16-bit wide
										 * indices for local variables. This
										 * opcode precedes an `ILOAD', e.g.. The
										 * opcode immediately following takes an
										 * extra byte which is combined with the
										 * following byte to form a 16-bit
										 * value.
										 */
	...
	public static final String accessToString(int access_flags,
		...
		int p = 0;
		for (int i = 0; p < Constants.MAX_ACC_FLAG; i++) { // Loop through known
															// flags
			p = pow2(i);
			if ((access_flags & p) != 0) {
				/*
				 * Special case: Classes compiled with new compilers and with
				 * the `ACC_SUPER' flag would be said to be "synchronized". This
				 * is because SUN used the same value for the flags `ACC_SUPER'
				 * and `ACC_SYNCHRONIZED'.
				 */
				if (for_class
						&& ((p == Constants.ACC_SUPER) || (p == Constants.ACC_INTERFACE))) {
					continue;
				}
	...
	public static final String codeToString(byte[] code,
			ConstantPool constant_pool, int index, int length, boolean verbose) {
	...
	public static final String codeToString(byte[] code,
			ConstantPool constant_pool, int index, int length) {
	...
	public static final String codeToString(ByteSequence bytes,
		...
		case Constants.TABLESWITCH:
			...
			buf.append("\tdefault = ").append(default_offset)
					.append(", low = ").append(low).append(", high = ")
					.append(high).append("(");
		...
		case Constants.LOOKUPSWITCH: {
			...
			default_offset += offset;
			buf.append("\tdefault = ").append(default_offset)
					.append(", npairs = ").append(npairs).append(" (");
			...
			break;
		/*
		 * Two address bytes + offset from start of byte stream form the jump
		 * target
		 */
		case Constants.GOTO:
		case Constants.IFEQ:
		...
		case Constants.IF_ICMPNE:
			buf.append("\t\t#").append(
					(bytes.getIndex() - 1) + bytes.readShort());
			break;
		/*
		 * 32-bit wide jumps
		 */
		...
		case Constants.JSR_W:
			buf.append("\t\t#").append(
					((bytes.getIndex() - 1) + bytes.readInt()));
			break;
		/*
		...
		case Constants.NEWARRAY:
			buf.append("\t\t<").append(Constants.TYPE_NAMES[bytes.readByte()])
					.append(">");
			break;
		/*
		...
		case Constants.PUTSTATIC:
			...
			buf.append("\t\t")
					.append(constant_pool.constantToString(index,
		...
		case Constants.INSTANCEOF:
			...
			buf.append("\t<")
					.append(constant_pool.constantToString(index,
							Constants.CONSTANT_Class)).append(">")
					.append((verbose ? " (" + index + ")" : ""));
			break;
		/*
		...
		case Constants.INVOKEINTERFACE:
			...
			buf.append("\t")
					...
					.append(verbose ? " (" + index + ")\t" : "").append(nargs)
					.append("\t").append(bytes.readUnsignedByte()); // Last byte
																	// is a
																	// reserved
																	// space
			break;
		...
		case Constants.LDC2_W:
			index = bytes.readUnsignedShort();
			buf.append("\t\t")
					.append(constant_pool.constantToString(index, constant_pool
		...
		case Constants.ANEWARRAY:
			index = bytes.readUnsignedShort();
			buf.append("\t\t<")
					.append(compactClassName(constant_pool.getConstantString(
	 ...
	 * prefix <em>java.lang</em> is also removed.
	 * 
	 * @param str
	 *            The long class name
	 * @param chopit
	...
	public static final String methodSignatureToString(String signature,
			String name, String access, boolean chopit) {
		return methodSignatureToString(signature, name, access, chopit, null);
	}

	/**
	 * A return type signature represents the return value from a method. It is
	 * a series of bytes in the following grammar:
	 * 
	 ...
	 * the value that it returns. <method_signature> ::= (<arguments_signature>)
	 * <return_signature> <arguments_signature>::= <argument_signature>*
	 * 
	 ...
	 */
	public static final String methodSignatureToString(String signature,
			String name, String access, boolean chopit, LocalVariableTable vars)
			throws ClassFormatException {
		StringBuffer buf = new StringBuffer("(");
		String type;
		int index;
		...
		try { // Read all declarations between for `(' and `)'
			...
			index = 1; // current string position
			while (signature.charAt(index) != ')') {
				String param_type = signatureToString(
						signature.substring(index), chopit);
		...
		buf.append(")");
		return access + ((access.length() > 0) ? " " : "") + // May be an empty
																// string
	...
	public static final String signatureToString(String signature,
			boolean chopit) {
		// corrected concurrent private static field acess
		wrap(consumed_chars, 1); // This is the default, read just one char like
									// `B'
		try {
	...
	private static int countBrackets(String brackets) {
		...
		for (int i = 0; i < chars.length; i++) {
			...
			case '[':
				if (open) {
					throw new RuntimeException("Illegally nested brackets:"
							+ brackets);
				}
				open = true;
				break;
	 ...
	 */
	public static final String format(int i, int length, boolean left_justify,
			char fill) {
	 ...
	 */
	public static final String fillup(String str, int length,
			boolean left_justify, char fill) {
	...
	public static final String printArray(Object[] obj, boolean braces,
		...
		for (int i = 0; i < obj.length; i++) {
			if (obj[i] != null) {
				buf.append((quote ? "\"" : "")).append(obj[i].toString())
						.append((quote ? "\"" : ""));
			} else {
				buf.append("null");
			}
