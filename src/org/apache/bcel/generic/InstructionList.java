...
public class InstructionList implements Serializable {
	...
	public InstructionList(byte[] code) {
		...
		try {
			while (bytes.available() > 0) {
				...
				pos[count] = off;
				/*
				 * Read one instruction from the byte stream, the byte position
				 * is set accordingly.
				 */
		...
		for (int i = 0; i < count; i++) {
			if (ihs[i] instanceof BranchHandle) {
				...
				if (bi instanceof Select) { // Either LOOKUPSWITCH or
											// TABLESWITCH
					Select s = (Select) bi;
	...
	public void setPositions(boolean check) {
		...
		for (InstructionHandle ih = start; ih != null; ih = ih.next) {
			 ...
			 * because BranchInstructions may have variable length depending on
			 * the target offset (short vs. int) or alignment issues
	...
	public InstructionList copy() {
		 ...
		 * and associate old instruction references with the new ones, i.e., a
		 * 1:1 mapping.
		 */
		for (InstructionHandle ih = start; ih != null; ih = ih.next) {
			Instruction i = ih.instruction;
		...
		while (ih != null) {
			...
			if (i instanceof BranchInstruction) {
				...
				if (bi instanceof Select) { // Either LOOKUPSWITCH or
					...
					for (int j = 0; j < itargets.length; j++) { // Update all
																// targets
	...
	/**
	 * Delete contents of list. Provides besser memory utilization, because the
	...
	public void update() {
		if (observers != null) {
			for (Iterator<InstructionListObserver> e = observers.iterator(); e
					.hasNext();) {
				e.next().notify(this);
			}
		}
