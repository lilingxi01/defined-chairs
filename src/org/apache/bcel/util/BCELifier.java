...
public class BCELifier extends org.apache.bcel.classfile.EmptyVisitor {
	...
	public void visitJavaClass(JavaClass clazz) {
		...
		_out.println("    _cg = new ClassGen(\""
				+ (("".equals(package_name)) ? class_name : package_name + "."
	...
	static String printFlags(int flags, int reason) {
		...
		for (int i = 0, pow = 1; pow <= Constants.MAX_ACC_FLAG; i++) {
			if ((flags & pow) != 0) {
				if ((pow == Constants.ACC_SYNCHRONIZED)
						&& (reason == FLAG_FOR_CLASS)) {
					buf.append("ACC_SUPER | ");
				} else if ((pow == Constants.ACC_VOLATILE)
						&& (reason == FLAG_FOR_METHOD)) {
					buf.append("ACC_BRIDGE | ");
				} else if ((pow == Constants.ACC_TRANSIENT)
						&& (reason == FLAG_FOR_METHOD)) {
					buf.append("ACC_VARARGS | ");
				} else {
					buf.append("ACC_")
							.append(Constants.ACCESS_NAMES[i]
									.toUpperCase(Locale.ENGLISH)).append(" | ");
				}
			}
			pow <<= 1;
		}
	...
	static String printType(String signature) {
		...
		if (t <= Constants.T_VOID) {
			return "Type."
					+ Constants.TYPE_NAMES[t].toUpperCase(Locale.ENGLISH);
